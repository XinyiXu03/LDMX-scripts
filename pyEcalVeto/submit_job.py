...
def main():
    ...
    command = 'singularity run --no-home --bind /home/xinyi_xu:/home/xinyi_xu ${LDMX_PRODUCTION_IMG} . python3 %s -o %s -m 3000000000000 ' % (script, outdir)
    ...
    for group in range(0,groups):
        ...
        for job in range(0,jobs):
            ...
            with open('%s/slurm_submit_%d.job' % (jobdir,job), 'w') as f:
                ...
                f.write('#SBATCH --mail-user=xinyi_xu@ucsb.edu\n')
                f.write('cd $SLURM_SUBMIT_DIR\n\n')
                f.write('/bin/hostname\n\n')
                f.write('cat /etc/centos-release\n\n')
                f.write ('module load singularity\n\n')
                f.write ('export LDMX_PRODUCTION_IMG=/home/xinyi_xu/ldmx_production_latest.sif\n\n')
                f.write('singularity run --no-home --bind /home/xinyi_xu:/home/xinyi_xu ${LDMX_PRODUCTION_IMG} .\n\n')
                #f.write ('chmod +x "$0"\n\n')
                f.write('%s -g data_flatout_more1%s\n' % (specific_command,a))
